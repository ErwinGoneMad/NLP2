...
def generate_revision_llama_cpp(
    llm: Any,
    original_poem: str,
    critique: str,
    original_prompt: str,
...
- La structure formelle ({form_spec.get("form", "poème")} de {form_spec.get("total_lines", "")} lignes EXACTEMENT)
...
Génère une version révisée améliorée qui répond aux suggestions de la critique :"""
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a skilled poet. Revise your work based on constructive feedback. Write in French.",
        },
...
def generate_revision(
    ...
    if model_type == "generic":
        return generate_revision_llama_cpp(
            model, original_poem, critique, original_prompt, form_spec
        )
    elif model_type == "finetuned":
        if tokenizer is None:
            raise ValueError("Tokenizer required for finetuned model")
        result = generate_revision_unsloth(
            model, tokenizer, original_poem, critique, original_prompt, form_spec
        )
        if result is None:
